...
import * as Benchmark from "benchmark-util";
...
if (save) {
  console.log(
    "\nYou need install the puppeteer to generate the graphic charts!\n"
  );
}
...
async function testSize(name, packet) {
  ...
  if (save) await generateGraphic("Packet size", name, fsizes);
}
...
async function testEncoder(name, packet) {
  ...
  if (save) await generateGraphic("Encoder Op/sec", name, opsec);
}
...
async function testDecoder(name, packet) {
  ...
  console.log(`Fastest is: ${fastest}\n\n`);
  if (save) await generateGraphic("Decoder Op/sec", name, opsec);
}
