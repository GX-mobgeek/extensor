...
const { buildParser, withAuth, forceOne } = require('../lib');
...
function client(srv, nsp, opts) {
    ...
    return ioc(url, { ...opts, parser: buildParser(map) });
}
...
describe('parser', function () {
    ...
    it('should be able to serialize and acknowledgement packets', function (done) {
        ...
        clientSocket.on("connect", () => {
            ...
            clientSocket.emit("chatMsg", { content: "perfOnTop", ts: Math.round(Date.now() / 1000) }, response => {
...
describe('force one connection', function () {
    ...
    it('should be able to deny based with authorization and id namespace provided', function (done) {
        ...
        try {
            ...
            withAuth(clientSocketConn1, { method: withAuth.CREDENTIAL }, authorize => {

                authorize({ login: 'ferco', pw: '' })

            });

            withAuth(clientSocketConn2, { method: withAuth.CREDENTIAL }, authorize => {
